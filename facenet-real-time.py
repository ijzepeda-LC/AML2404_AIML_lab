...
face_cascade = cv2.CascadeClassifier('models_weights/haarcascade_frontalface_default.xml')
...
model = model_from_json(open("models_weights/facenet_model.json", "r").read())
print("model built")

#https://drive.google.com/file/d/1971Xk5RwedbudGgTIrGAL4F7Aifu7id1/view?usp=sharing
model.load_weights('models_weights/facenet_weights.h5')
...
#put your celeb pictures in this path as name_of_celeb.jpg
celeb_pictures = "actors_db/"

celebs = dict()

for file in listdir(celeb_pictures):
	celeb, extension = file.split(".")
	img = preprocess_image('actors_db/%s.jpg' % (celeb))
	# img = preprocess_image('database/%s.jpg' % (celeb))
	representation = model.predict(img)[0,:]
	
	celebs[celeb] = representation
	
print("celeb representations retrieved successfully")
...
while(True):
	...
	for (x,y,w,h) in faces:
		if w > 130: #discard small detected faces
			...
			#celeb dictionary is using preprocess_image and it normalizes in scale of [-1, +1]
			img_pixels /= 127.5
			img_pixels -= 1
			...
			for i in celebs:
				celeb_name = i
				source_representation = celebs[i]
							#The faces of all database, has been processed before
				distance = findEuclideanDistance(captured_representation, source_representation)
				
				print(celeb_name,": ",distance)
			...
			for i in celebs:
				...
				if index == np.argmin(distances):
					if distances[index] <= threshold:
						#print("detected: ",celeb_name)
						
						#label_name = "%s (distance: %s)" % (celeb_name, str(round(distance,2)))
						similarity = 100 + (20 - distance)
						if similarity > 99.99: similarity = 99.99
						
						label_name = "%s (%s%s)" % (celeb_name, str(round(similarity,2)), '%')
